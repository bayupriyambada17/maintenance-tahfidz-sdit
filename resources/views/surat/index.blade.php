    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                <a href="{{ url('/surat/tambah') }}" class="btn btn-sm btn-primary float-right">+ Tambah</a>
                ...
                <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form action="{{ url('/surat/import') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                    ...
                    <tbody>
                        @foreach ($data as $d)
                            <tr>
                                ...
                                <td>{{ $d->juz }}</td>
                                <td>{{ $d->order }}</td>
                                <td>{{ $d->last_ayat - $d->first_ayat + 1 }}</td>
                                <td>
                                    <a href="{{ url('/surat/edit/'.$d->id) }}" title="Edit" class="btn btn-sm btn-info"><i class="fa fa-solid fa-edit"></i></a>
                                    <form action="{{ url('/surat/delete/'.$d->id) }}" method="post" class="d-inline">
                                        @method('delete')
                                        @csrf
